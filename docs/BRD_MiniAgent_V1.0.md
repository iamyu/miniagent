...
│   ├── chat.py             # ChatEngine（对话+工具循环+技能注入）
│   ├── tools.py            # 10 个内置工具 + ToolRegistry
│   ├── skills.py           # SkillsLoader + Skill
│   ├── config.py           # 三级配置管理
│   ├── web.py              # FastAPI 后端 + WebSocket
│   └── static/             # Web UI 静态文件
│       ├── index.html      # 主页面
│       ├── styles.css      # 样式
│       └── app.js          # 交互逻辑
