...
2. `$env:USERPROFILE\.claude\skills\document-skills\pptx\scripts\html2pptx.js`（Claude Code 内置 pptx skill）
3. `$env:USERPROFILE\.workbuddy\skills\pptx\scripts\html2pptx.js`（WorkBuddy pptx skill）
...
将 `html2pptx.js` 复制到项目的 `scripts/` 目录（PowerShell 5.1 格式）。以下命令会自动检测 `shell-pptx` 的常见安装位置，无需手动修改：

```powershell
# 自动检测 shell-pptx skill 目录（项目级优先，其次 .claude / .lingma / .workbuddy）
$skillDir = @(
    "$pwd\skills\shell-pptx",
    "$env:USERPROFILE\.claude\skills\shell-pptx",
    "$env:USERPROFILE\.lingma\skills\shell-pptx",
    "$env:USERPROFILE\.workbuddy\skills\shell-pptx"
) | Where-Object { Test-Path "$_.\scripts\html2pptx.js" } | Select-Object -First 1

if (-not $skillDir) {
    throw "未找到 shell-pptx skill 目录。请确认 skill 已正确安装。"
}

Copy-Item "$skillDir\scripts\html2pptx.js" "$projectName\scripts\"
```
...
复制模板（PowerShell 5.1 格式）。以下命令会自动检测 `shell-pptx` 的常见安装位置，无需手动修改：
...
$skillDir = @(
    ...
    "$env:USERPROFILE\.lingma\skills\shell-pptx",
    "$env:USERPROFILE\.workbuddy\skills\shell-pptx"
) | Where-Object { Test-Path "$_.\scripts\build.js" } | Select-Object -First 1

if (-not $skillDir) {
    throw "未找到 shell-pptx skill 目录。请确认 skill 已正确安装。"
}

Copy-Item "$skillDir\scripts\build.js" "$projectName\build.js"
```
...
### Step 6: 构建 PPTX（必须自动执行）

**CRITICAL: DO NOT ask the user whether to run the build. After all slide HTML files and build.js are ready, you MUST automatically execute the build command yourself.**

```powershell
cd my-ppt; node build.js
```

**✅ 构建成功后，告知用户 PPTX 文件路径。若失败，检查错误信息并提示用户修复依赖（npm install pptxgenjs playwright sharp），如果 npm 包缺失，自动执行安装后重新构建。**
